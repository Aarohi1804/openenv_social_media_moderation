...
class SocialMediaModerationEnvironment(Environment):
    ...
    def step(self, action: ModerationAction) -> ModerationObservation:
        ...
        if self._consecutive_same_action >= 5: raw_reward -= 0.2
            
        self._current_post_index += 1
        total_posts = self.config["total_posts"]
        episode_done = (self._current_post_index >= total_posts or self._cumulative_harm >= 5.0)
    ...
    def _generate_posts(self) -> List[Dict]:
        ...
        for i in range(total):
            ...
            if is_campaign:
                virality, spread_vel, misinfo_prob = 0.99, 0.99, 0.98
            ...
            posts.append({
                ...
                "misinfo_probability": round(misinfo_prob, 3), "virality_score": round(virality, 3),
                "spread_velocity": round(spread_vel, 3), "report_count": reps, "trusted_report_count": treps,
                "reporter_trust": round(trust, 3), "user_credibility": random.uniform(0.1, 0.9),
                "is_repeat_offender": is_repeat, "content_category": random.choice(CONTENT_CATEGORIES)
            })
        return posts

    def _calculate_reward(self, action: str, post: Dict) -> float:
        is_fake, is_repeat, virality = post["is_fake"], post["is_repeat_offender"], post["virality_score"]
        reward = 0.0
        if is_fake:
            if action == "DELETE": reward = (1.2 if is_repeat else 1.0) + (virality * 0.5)
            elif action == "ALLOW": reward = -1.5 if is_repeat else -1.0
            else: reward = 0.5
        else:
    ...
    def _update_tracking(self, action: str, post: Dict) -> None:
        ...
        if action in ["DELETE", "REDUCE_REACH"] and post["is_fake"]:
        ...
        elif action == "ALLOW" and not post["is_fake"]:
    ...
    def _resolve_escalations(self):
        to_resolve = [idx for idx, step in self._escalated_posts.items() if self._state.step_count - step >= 2]
        for idx in to_resolve: del self._escalated_posts[idx]

    def _get_current_factcheck(self, index: int) -> float:
        if index in self._escalated_posts: return 1.0
        return min(0.95, 0.1 + (self.config["factcheck_rise_rate"] * self._state.step_count))
    ...
    def _make_observation(self, index: int, reward: float, done: bool) -> ModerationObservation:
        ...
        return ModerationObservation(
            ...
            environmental_warning="0.8" if post["is_brigaded"] else "0.1", # Type-safe string
    ...
    def get_grader_score(self) -> float:
        ...
        if self.task_id == "task_easy": score = (0.6 * fake_rate) + (0.4 * real_rate)
        elif self.task_id == "task_medium": score = (0.5 * fake_rate) + (0.35 * real_rate) + (0.15 * (self._early_actions / max(1, self._total_fake)))
        else: score = (0.4 * (self._campaign_posts_actioned / max(1, self.config["campaign_size"]))) + (0.35 * fake_rate) + (0.25 * real_rate)
